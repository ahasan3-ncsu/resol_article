...
thermal spike in the supercell is varied to account for both on-centered and
off-centered collisions between the gas bubble and thermal spike.
...
R + R_{spike}$. Three different bubble sizes (15 \r{A}, 25 \r{A}, and
...
Thus, heterogeneous the resolution rate $b_{het}$ can be writted as
\begin{align}
	b_{het}
	&= \dot{F} \sum_{i=1}^2 \int_{r=0}^{\infty}
		\bigg( \frac{\chi_i}{\chi_{0,i}} \bigg)
	...
	&= 0.225 \pi r_c^2 \dot{F} \sum_{i=1}^2 \int_{x=0}^{\infty}
		[1 - \exp(-5.1 \zeta S_{e,i} / R^{2.2})] dx 
\end{align}
...
bubble radius $R$ using equations \ref{eq:dx}, \ref{eq:xe}, and \ref{eq:sr}.
Figure \ref{fig:res} displays the result from this work along with the
prediction from the DART model. The calculated re-solution rate is about two
orders of magnitude higher than the DART prediction for very small bubbles.
This difference is about three orders of magnitude for large bubbles. Only for
bubbles of radius around 50 \r{A}, the difference is less than one order of
magnitude. The value of $\zeta$ does not impact this comparison since the ratio
between the re-solution rate at $\zeta=1$ is at most double of the rate at
$\zeta=0.55$.

\begin{figure}[ht]
	\centering
	\centering
	\includegraphics[width=10cm]{images/resRate_ext.pdf}
	\caption{Xe gas bubble re-solution rate in U-10Mo as a function of bubble
	radius at a fission rate of $10^{14}$ fiss/cm$^3$/s.}
	\label{fig:res}
\end{figure}

------------------------------------
{\color{cyan} Done till this point}
------------------------------------

% Use the presentations. You've made a lot of them. Start over. Use the marker.
% Go sequentially.
% Add more lines about zeta. Add a table as well.
% The fitting would be problematic.

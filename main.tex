...
So far, we only have the $S_e$ value for Xe in U-10Mo. $S_{e,Xe}$ as a function
of the distance traversed by the Xe atom is shown in Figure \ref{fig:srim}.
This data is generated using the SRIM software \cite{ziegler2010srim}. The
following model is fit to represent the $S_{e,Xe}$ data in the integral in
equation \ref{eq:dx}. The re-solution fraction calculated for only Xe (one out
of many fission products) is multiplied by 2 (total fission yield) to get an
approximation for the total re-solution fraction.

\begin{align}
	S_{e,Xe} = 28.1 \exp(-0.222 x^{1.62})
		+ 1.72 \exp(-2.16 \times 10^{-7} x^{8.21})
\end{align}

\begin{figure}[ht]
	\centering
	\includegraphics[width=10cm]{images/srim.png}
...
range of plausible fission rates. The re-solution rate used in DART is 2.5
...
$b = 8 \times 10^{-19} \dot{F}$, where $\dot{F}$ is the fission rate specified
as fissions/cm$^3$/s.

...
A 160$\times$160$\times$160 \r{A}$^3$ supercell is used for the PKA model. This
amounts to $\sim$8 million atoms in the system. Figure \ref{fig:struct} shows
the cross-section of such a supercell. The system is equilibrated at 400 K at a
pressure of 0 bar within the NPT ensemble using the Nose-Hoover barostat and
thermostat. Using the resulting configurations, spherical bubbles of 4 nm
diameter are created by removing a stoichiometric number of uranium and
...
constant volume simulation cell with a Nose-Hoover thermostat at the edges of
...
The thermal spike method is a powerful model used to describe the interaction
...
model, the supercell is 160 × 160 × 80 \r{A}$^3$, which holds $\sim$4 million
atoms. The atoms present in a cylindrical zone of 2 nm radius along the thermal
spike axis are excited with a high kinetic energy to simulate local heating due
to electronic effects. A xenon bubble has been inserted with its center on the
...
is the thickness of the annulus within which all gas-atoms are knocked out. Ye
et al. reported the optimized values of $b_0$, $\lambda$, and  $R_{resol}$ in

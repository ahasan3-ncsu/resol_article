...
\begin{abstract}
	Molecular dynamics simulations are performed to quantify the re-solution
	rate of Xe gas bubble in $\gamma$U-10Mo fuel. Two models, the primary
	knock-on atom (PKA) and thermal spike models, are employed to evaluate
	homogeneous and heterogeneous re-solution, respectively. Notably,
	homogeneous re-solution is found to be negligible compared to the
	heterogeneous re-solution. In the thermal spike simulations, Xe gas bubbles
	of varying radii and pressure are considered. The thermal spike axis is
	...
	re-solved into the $\gamma$U-10Mo matrix is evaluated as a function of
	bubble radius, off-centered distance, thermal spike energy ($S_{e,eff}$),
	and Xe/vacancy ratio. Additionally, the electronic stopping power ($S_e$)
	of fission products was simulated to determine the energy imparted by the
	fission products at specific distances. To further refine the analysis, a
	fraction ($\zeta$) of the energy imparted by fission products is equated to
	the thermal spike energy ($\zeta S_e = S_{e,eff}$). Finally, using all this
	information, an overall Xe gas bubble re-solution rate in the
	$\gamma$U-10Mo fuel is computed as a function of bubble radius, Xe/vacancy
	ratio and fission rate, considering reasonable $\zeta$ values.
\end{abstract}
...
cascades and thermal spikes, a process known as the re-solution. The relative
rates of the re-solution determines the overall size and density of the bubbles
\cite{ye2023, olander2006re, parfitt2008}, which in turn delays the onset of
fission gas release and impacts the final gas release rate. The re-solution of
...
re-solution mechanism, atoms from the gas bubbles are ejected individually
through collisions with fission products or the recoil of U atoms that traverse
the bubbles. These atomic collision cascades are primarily governed by the
nuclear stopping power of the material. In the heterogeneous model, entire gas
bubbles are dissolved by a passing fission fragment in the vicinity. The
driving mechanism is the local heating of the material containing gas bubbles
through electronic stopping of the fission fragment \cite{setyawan2018}. Since
both these mechanisms occurs on a short timescale, it is challenging to
determine re-solution rates that contribute to the fission gas release models
through experiments. Therefore, atomistic scale modelling is not only necessary
to determine the re-solution rate but also to elucidate the fundamental
mechanism of re-solution in $\gamma$U-10Mo.
...
bubbles. In 2009, Schwen et al. \cite{schwen2009md} investigated the
homogeneous re-solution of Xenon (Xe) gas bubbles in UO$_2$ using a binary
collision model and molecular dynamics (MD) simulations. The following year,
Huang et al. \cite{huang2010md} endeavored to examine the impact of thermal
spikes on Xe re-solution in UO$_2$. In 2012, Govers et al. \cite{govers2012}
evaluated both the PKA and thermal spike models for Xe gas bubbles in UO$_2$
and also proposed a mathematical model for the re-solution rate. However, the
most comprehensive work on Xe gas bubble re-solution in UO$_2$ was conducted by
...
Their findings suggest that heterogeneous re-solution of gas bubbles is the
dominant method of re-solution in UO$_2$. In addition to UO$_2$, re-solution
rate of fission gas bubbles has also been evaluated in uranium carbide (UC) by
Matthews et al. \cite{matthews2015diss} using binary collision methods. The
thermal spike model was not employed in UC because the local heating does not
exceed the melting temperature \cite{matthews2015diss, ronchi1986}. In summary,
the binary collision model and MD simulations were used to determine the
re-solution rate in nuclear fuels. In this work, we employ MD simulations to
model the homogeneous and heterogeneous mechanism of re-solution and determine
the re-solution rates in $\gamma$U-10Mo.
...
events that may not significantly influence the re-solution process. However,
binary collision model needs to be utilized in this approach to get a energy
spectrum of the gas atom PKAs first \cite{schwen2009md}. One challenge of
modeling homogeneous re-solution with MD simulations is the channeling of PKAs
or their recoils over long distances without any collisions \cite{jarrin2021}.
As a result, collecting statistics on PKA interactions with gas bubbles can be
...
\cite{stoller2000}. Therefore, in this work, we direct the PKAs in a particular
direction to induce maximum disorder and to avoid channeling. For the MD
simulations of the heterogeneous re-solution, the thermal spike model is useful
for describing the interaction between the fission fragments and fuel. These
interactions occur primarily through the electronic stopping of the energetic
...
fission gas swelling in $\gamma$U-Mo in different operational situations. One
...
rigorous. Therefore, in this study, we investigate the re-solution of Xe gas
bubble in $\gamma$U-Mo fuel using MD simulations by considering both
homogeneous and heterogeneous re-solution mechanisms by simulating PKAs and
thermal spikes, respectively. We also calculate the electronic stopping power
of representative fission products to determine the energy imparted on gas
bubbles by fission events at a certain distance. Finally, we evaluate an
overall re-solution rate of Xe gas bubbles as a function of both bubble radius,
...
re-solution data is gathered and plotted in subfigure \ref{fig:frac}(a) as a
...
different bubble sizes are plotted in \ref{fig:frac}(b) against bubble radius.
...
atoms, which is also apparent in subfigure \ref{fig:frac}(a). One possible
...
separation of Xe atoms from the bubble. Since the thermal spike energy dictates
the volume of these low density regions, it also affects the total number of Xe
atoms that can stay in a low energy configuration outside the original bubble.
...
Re-solution rate can be defined as the probability of a Xe atom in a gas bubble
getting re-solved into the fuel matrix per unit time. The heterogeneous
re-solution rate can thus be calculated using the fraction of re-solved atoms
in a bubble due to a thermal spike and the number of thermal spikes occurring
per second. Both bubble size and pressure are going to be considered in the
calculation of the bubble re-solution rate. As we have stated in the previous
subsection, the number of re-solved Xe atoms remains constant with Xe/vacancy
ratio. Therefore, the fraction of re-solved Xe atoms (number of re-solved Xe
atoms divided by the total number of Xe atoms in the system) is inversely
...
the nominal $\phi$ value of 0.2. We are also defining $f(R_{bubble}, \dot{F})$
...
typical fission product yield graph \cite{setyawan2018, mills1995}. $S_{e,Xe}$
and $S_{e,Sr}$ as a function of the distance traversed by the fission product
is shown in Figure \ref{fig:elec}. This data is generated using the SRIM
software. The following model is fit to represent the $S_e$ data.
\begin{align}
	\label{eq:xe}
...
\begin{figure}[ht]
	...
	\caption{
		...
		bubble radius at a fission rate of $10^{14}$ fiss/cm$^3$/s for a
...
bubble size, bubbles having more Xe atoms (and thus more pressure) will have a
lower fraction of re-solved atoms. This leads to the following simple
expression.
\begin{align}
	g(\phi) &= \frac{0.2}{\phi}
\end{align}

The Xe/vacancy ratio, $\phi$ can be used to get the molar volume of Xe, $v$. At
400 K, the equilibrium volume of $\gamma$U-Mo is $19.7$ \r{A}$^3$/atom.
Therefore, $v$ can be expressed the following way.
\begin{align}
	v
...
radius and Xe/vacancy ratio. The re-solution rate decreases with both of the
variables.
...
swelling in $\gamma$U-10Mo. The mesoscale program DART incorporates this
parameter for such calculations \cite{ye2023}. The currently used re-solution
rate $b_{dart}$ in DART is defined as follows.
\begin{align}
...
bubble radius $\lambda$. The rationale behind the piecewise implementation of
the re-solution rate is to delineate the intragranular bubbles from
intergranular bubbles.

The re-solution rate calculated in this work uses only the simulation of
intragranular bubbles. We assume that the trapping effect of the grain
boundaries is not significant in the timescale of thermal spikes and thus it is
not considered. To model gas bubble evolution, re-solution rate and trapping
rate may be implemented separately in higher length scale programs. That would
allow greater freedom in modeling complex behavior of gas bubbles in the fuel
under different contexts. For example, the grain boundary diffusion coefficient
of Xe in $\gamma$U-Mo can be 15 orders of magnitude higher than the intrinsic
diffusion coefficient of Xe in $\gamma$U-Mo around 600 K \cite{hasan2024gb}.
Therefore, the Xe trapping rate for intergranular bubbles can be set 15 times
higher than the trapping rate for intragranular bubbles. Even with subtle
differences between the definitions of re-solution in this work and in DART, we
choose to compare the re-solution rates. Figure \ref{fig:dart} shows the
...
in describing the overall re-solution behavior. Simulations of both on-centered
and off-centered thermal spikes are conducted to get the spatial correlation
between re-solution and thermal spike distance. The Xe/vacancy ratio in the
bubble is also varied to understand the role of bubble pressure. Additionally,
the electronic stopping power of fission products (Xe and Sr) in $\gamma$U-10Mo
is simulated to provide distance-dependent electronic stopping power. All this
information is then used to calculate the overall re-solution rate of Xe gas
bubbles in the fuel. The re-solution rate of intragranular Xe gas bubbles under
equilibrium pressure in $\gamma$U-10Mo is found to be on the order of $10^{-4}$
to $10^{-2}$ s$^{-1}$ for a fission rate of $10^{14}$ fissions/cm$^3$/s. An
analytical form of re-solution rate dependent on bubble radius, bubble
pressure, and fission rate is also provided. The result from this work will
inform higher length scale models of $\gamma$U-10Mo with a physics-based
description of Xe gas bubble re-solution rate.

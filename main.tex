...
in uranium carbide (UC) by Matthews et al. \cite{matthews2015diss} in 2015
...
due to high electronic conductivity and thermal diffusivity
compared to UO$_2$ \cite{matthews2015diss, ronchi1986, mao2025}.
...
where $\gamma = \num{4e-9}$ eV/(K$^2$e).
The electronic density is set to $\rho_e = 0.625$ e/\r{A}$^3$,
and the thermal conductivity is equated to $\kappa_e = D_e \rho_e C_e$,
where $D_e = 10$ \r{A}$^2$/fs is the thermal diffusion coefficient
...
\begin{figure}[!ht]
	...
	\begin{subfigure}{\textwidth}
		\centering
		\caption{}
		\includegraphics[width=6cm]{images/coord.pdf}
	\end{subfigure}
	\caption{
...
so that all fission tracks point in the same direction (Figure \ref{fig:coord}a).
...
\begin{figure}[!ht]
	...
	\end{subfigure}
	\caption{
		(a) FF tracks from 100 simulations of \Y ion in U-10Mo.
		(b) Surface discretization of FF information across volume.
	}
	\label{fig:fftrack}
\end{figure}
% need details
FF tracks \ref{fig:fftrack}.
...
\begin{figure}[!ht]
	...
	\caption{
		Simulation of a 5 MeV \Y incident on a 64 nm radius Xe bubble.
		Total Xe recoils: 3823.
		Outside the bubble: 155.
		Re-solved Xe atoms: 9.
	}
	\label{fig:ffbub}
\end{figure}
FF-bubble interaction \ref{fig:ffbub}.
...
to obtain re-solution rates for specific bubble radii,
...
Since $\chi$ is the ratio between the number of re-solved atoms
...
From here on, $b_{eq}$ will be used to describe re-solution

...
\begin{abstract}
	...
	re-solution rate in U-10Mo fuel. Primary knock-on atom (PKA) and thermal
	spike models are employed to evaluate homogeneous and heterogeneous
	...
	simulations are thus performed for Xe gas bubbles of different radii
	...
	U-10Mo matrix is evaluated as a function of bubble radius ($R_{bubble}$),
	off-centered distance ($r$), and thermal spike energy ($S_{e,eff}$). The
	electronic stopping power ($S_e$) of fission products are also simulated to
	find the energy imparted by the fission products at a certain distance. A
	fraction ($\zeta$) of the energy imparted by fission products is equated to
	the thermal spike energy ($\zeta S_e$). Eventually, all the information is
	used to compute an overall Xe gas bubble re-solution rate in the U-10Mo
	fuel as a function of bubble radius for reasonable $\zeta$ values.
\end{abstract}
...
% UMo necessity
% Re-solution rate necessity
% Rate in other fuels. what did they do?
% PKA and thermal spike need explainers
% DART model needs fluff
...
\cite{meyer2014}. However, the U-Mo monolithic fuel exhibits excessive swelling
during operation \cite{beeler2018gb}. To understand the fuel evolution under
irradiation, mesoscale and engineering level fuel performance models require
the knowledge of fundamental mechanistic behavior of fission products in the
fuel. As such, understanding the progression of Xe gas bubbles in the fuel is
crucial. The Xe gas bubbles act as a sink for individual Xe atoms (trapping of
Xe) and subsequently grow in size after absorption. Under irradiation, the Xe
atoms in the gas bubble are thrown back into the fuel matrix by fission
product-induced cascades and thermal spikes (re-solution of Xe), and this
process reduces the bubble size \cite{ye2023}.

% need something for collision cascades
% also need to describe the relationship between the two models
% what is necessary when?
% the paragraph below looks like copy/paste. I DIDN"T WRITE IT!

Re-solution of fission gas two commonly accepted mechanisms: homogeneous
re-solution and heterogeneous re-solution \cite{olander2006re}. In the
homogeneous re-solution mechanism, the atoms from the gas bubbles are ejected
one at a time via atomic collision cascades, a.k.a. nuclear stopping. In the
heterogeneous mechanism, electronic stopping raises the local temperature above
the melting temperature and destroys the gas bubble en bloc
\cite{setyawan2018}. These two mechanisms can be simulated using molecular
dynamics. To simulate homogeneous re-solution, an arbitrary atom in the system
can be given a high kinetic energy so that it acts as a primary knock-on atom
(PKA). Simulating heterogeneous re-solution requires emulating a fission track
by raising the temperature of atoms in a cylindrical region. This is also
called the thermal spike method.
...
swelling in U-10Mo. The mesoscale program Dispersion Analysis Research Tool
(DART) incorporates this parameter for such calculations \cite{ye2023}. The
currently used re-solution rate $b_{dart}$ in DART is defined as follows.
\begin{align}
...
The parameters $b_0$, $\lambda$, and $R_{resol}$ are considered adjustable, and
the optimized values are $b_0 = 2 \times 10^{-18}$ cm$^3$, $\lambda = 5 \times
10^{-7}$ cm, and $R_{resol} = 3 \times 10^{-9}$ cm as reported in
\cite{ye2023}. Figure \ref{fig:dart} shows the re-solution rate for bubble
radius up to 100 \r{A} as used in DART. Since the parameters $\lambda$ and
$R_{resol}$ are not coupled, the re-solution rate is discontinous at bubble
radius $\lambda$, which is unphysical.

\begin{figure}[ht]
	\centering
	\includegraphics[width=7cm]{images/onlyDart.pdf}
...
and heterogeneous re-solution mechanism are investigated via the PKA and
thermal spike methods, respectively. Finally, the re-solution rate of Xe gas
bubbles is evaluated as a function of bubble radius.

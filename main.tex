...
\begin{figure}[ht]
	...
	\caption{
		...
		(b) placeholder for new illustration.
	}
	\label{fig:coord}
\end{figure}
...
Also, if a fission track produced by isotope $k$ interacts with a Xe gas bubble,
it re-solves $\xi_i$ fraction of the Xe atoms.
Each fission product isotope $k$ from fission events thus contributes
$\xi_k \dot{F} dV$ to the total re-solution of the bubble.
The re-solution rate can then be expressed as:
...
Why is $D = 1000$ \r{A}? \ref{fig:recoil}
...
% when are Xe atoms considered re-solved?
% add a graph to show the 1 nm cutoff
Counting re-solved atoms \ref{fig:xeres}
